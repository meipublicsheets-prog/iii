...
function searchBins(searchParams) {
  try {
    ...
    // Use flexible header matching to find column indices
    const findCol = (names) => {
    ...
    const binCodeIndex = findCol(['Bin_Code']);
    const binNameIndex = findCol(['Bin_Name']);
    const pushNumberIndex = findCol(['Push_Number']);
    const fbpnIndex = findCol(['FBPN']);
    const manufacturerIndex = findCol(['Manufacturer']);
    const projectIndex = findCol(['Project']);
    const initialQtyIndex = findCol(['Initial_Quantity']);
    const currentQtyIndex = findCol(['Current_Quantity']);
    const stockPercentageIndex = findCol(['Stock_Percentage']);
    const skidIdIndex = findCol(['Skid_ID']);
    const uomIndex = findCol(['UOM']);
    const skuIndex = findCol(['SKU']);
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      if (matches) {
        results.push({
          ...
          uom: uomIndex > -1 ? (row[uomIndex] || 'EA') : 'EA',
