...
/**
 * Creates a lookup map from Item_Master for fast retrieval of UOM by FBPN
 * Key: FBPN (uppercase) -> Value: { uom, mfpn, description }
 */
function getItemMasterMap_() {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        if (fbpn) {
          map[fbpn] = {
            ...
            uom: uomIdx > -1 ? String(data[i][uomIdx] || '') : ''
          };
        }
      }
    }
  ...
  // Supplement with Project_Master if missing
  const projSheet = ss.getSheetByName('Project_Master');
  if (projSheet && projSheet.getLastRow() > 1) {
    ...
    if (fbpnIdx > -1) {
      for (let i = 1; i < data.length; i++) {
        ...
        } else if (fbpn && map[fbpn] && !map[fbpn].uom && uomIdx > -1) {
          map[fbpn].uom = String(data[i][uomIdx] || '');
        }
      }
    }
  }
...
/**
 * Get UOM for a single FBPN
 * @param {string} fbpn - The FBPN to lookup
 * @param {Object} [itemMasterMap] - Optional pre-loaded map for batch operations
 * @returns {string} The UOM or empty string
 */
function getUOMForFBPN_(fbpn, itemMasterMap) {
  const map = itemMasterMap || getItemMasterMap_();
  const key = String(fbpn || '').toUpperCase().trim();
  return (map[key] && map[key].uom) || '';
}
...
function processCustomerOrder(orderData) {
  try {
    ...
    if (pickLogItems.length > 0) {
      ...
      // OutboundLog (one row per FBPN - aggregated across bins)
...
function addItemsToPickLog(ss, data) {
  ...
  // Pre-load UOM map for batch lookup
  const itemMasterMap = getItemMasterMap_();
  ...
  data.items.forEach(item => {
    ...
    const uom = item.uom || getUOMForFBPN_(item.fbpn, itemMasterMap);
...
// ============================================================================
// HELPER: OUTBOUNDLOG WRITER — ONE ROW PER FBPN PER ORDER (aggregated)
// Headers: Date Order_Number Task_Number Transaction Type Warehouse Company Project
//          FBPN Manufacturer Qty Skid_ID SKU UOM
// Note: Bin_Code column removed - aggregating by FBPN across all bins
// ============================================================================
  ...
  (data.items || []).forEach(item => {
    ...
    if (!byFbpn[fbpn]) {
      byFbpn[fbpn] = {
        ...
        sku: item.sku || '',
        uom: item.uom || getUOMForFBPN_(fbpn, itemMasterMap)
      };
    }
    byFbpn[fbpn].totalQty += qty;
    // Keep first non-empty manufacturer/sku/uom
    if (!byFbpn[fbpn].manufacturer && item.manufacturer) {
      byFbpn[fbpn].manufacturer = item.manufacturer;
    }
    if (!byFbpn[fbpn].sku && item.sku) {
      byFbpn[fbpn].sku = item.sku;
    }
    if (!byFbpn[fbpn].uom && item.uom) {
      byFbpn[fbpn].uom = item.uom;
    }
  });

  // Build rows - one per FBPN
  const rows = [];
  for (const fbpn in byFbpn) {
    if (!byFbpn.hasOwnProperty(fbpn)) continue;
    const agg = byFbpn[fbpn];
    ...
    rows.push([
      ...
      data.project,           // Project
      fbpn,                   // FBPN
      agg.manufacturer,       // Manufacturer
      agg.totalQty,           // Qty (combined total for this FBPN)
      '',                     // Skid_ID
      agg.sku,                // SKU
      agg.uom                 // UOM
    ]);
...
function writeRequestedItem(ss, itemData) {
  ...
  // Get UOM for this FBPN
  const uom = itemData.uom || getUOMForFBPN_(itemData.fbpn);
...
function createBackorder(ss, backorderData) {
  ...
  // Get UOM for this FBPN
  const uom = backorderData.uom || getUOMForFBPN_(backorderData.fbpn);
...
function createAllocation(ss, allocationData) {
  ...
  // Get UOM for this FBPN
  const uom = allocationData.uom || getUOMForFBPN_(allocationData.fbpn);
