...
// ============================================================================
// S-PATTERN DAILY CYCLE COUNT GENERATION
// ============================================================================

/**
 * Parses a bin code in format XX.X (e.g., A1.5, B2.38)
 * First char = Row (letter), Second char = Level (number)
 * After period = Skid sequence in that row
 * @param {string} binCode - The bin code to parse
 * @returns {Object|null} {row, level, sequence, original} or null if invalid
 */
function parseBinCode_(binCode) {
  if (!binCode || typeof binCode !== 'string') return null;
  ...
  return {
    row: match[1],           // Letter (A, B, C, etc.)
    level: parseInt(match[2], 10),  // Level (1 = ground, 2, 3, etc.)
    sequence: parseInt(match[3], 10), // Skid sequence in row
    original: code
  };
}

/**
 * Generates a daily cycle count batch using S-pattern selection.
 * 
 * Logic:
 * - Selects 75 bins per day (375 bins per 5-day work week)
 * - Rows are selected randomly
 * - Levels within a row are selected randomly
 * - S-Pattern: First row goes low→high (1→38), next row goes high→low (38→1)
 * - Tracks previously counted bins to avoid duplicates within the week
 * 
 * @param {Object} options - Optional configuration
 * @param {number} options.dailyTarget - Number of bins per day (default: 75)
 * @param {boolean} options.excludeRecent - Exclude bins counted in last 7 days (default: true)
 * @returns {Object} { success, batchId, bins, message }
 */
function generateDailyCycleCountBatch(options) {
  try {
    const cfg = getCycleConfig_();
    const ss = SpreadsheetApp.getActive();
    const binSheet = ss.getSheetByName(cfg.BIN_STOCK_SHEET);
    ...
    const dailyTarget = (options && options.dailyTarget) || 75;
    const excludeRecent = options && typeof options.excludeRecent !== 'undefined'
      ? options.excludeRecent
      : true;

    // Get all bins from Bin_Stock
    const binData = binSheet.getDataRange().getValues();
    const binHeaders = binData[0];
    ...
    if ([idxBinCode, idxFBPN, idxMan, idxProj, idxQty].some(i => i === -1)) {
      return { success: false, message: 'Bin_Stock missing required columns.' };
    }

    // Build set of recently counted bins (last 7 days)
    const recentlyCountedSet = new Set();
    if (excludeRecent) {
      const cycleData = cycleSheet.getDataRange().getValues();
      const cycleHeaders = cycleData[0];
      ...
      const idxManC = cycleHeaders.indexOf('Manufacturer');
      const idxProjC = cycleHeaders.indexOf('Project');
      const idxStatusC = cycleHeaders.indexOf('Status');
      ...
      for (let i = 1; i < cycleData.length; i++) {
        const r = cycleData[i];
        const status = r[idxStatusC] || '';

        // Include bins that are Open/In Progress or Completed recently
        if (status === 'Open' || status === 'In Progress') {
          const key = [
            (r[idxBinCodeC] || '').toString().toUpperCase(),
            (r[idxFBPNC] || '').toString().toUpperCase(),
            (r[idxManC] || '').toString().toUpperCase(),
            (r[idxProjC] || '').toString().toUpperCase()
          ].join('||');
          recentlyCountedSet.add(key);
        } else if (status === 'Completed' && r[idxCountedAt]) {
          const countedAt = new Date(r[idxCountedAt]);
          if (countedAt >= sevenDaysAgo) {
            const key = [
              (r[idxBinCodeC] || '').toString().toUpperCase(),
              (r[idxFBPNC] || '').toString().toUpperCase(),
              (r[idxManC] || '').toString().toUpperCase(),
    ...
    // Parse and organize all available bins by row
    const rowMap = {}; // { 'A': { levels: { 1: [{bin}], 2: [{bin}] } } }
    const availableBins = [];

    for (let i = 1; i < binData.length; i++) {
      const row = binData[i];
      const binCode = (row[idxBinCode] || '').toString().trim().toUpperCase();
      const fbpn = (row[idxFBPN] || '').toString().toUpperCase();
      const man = (row[idxMan] || '').toString().toUpperCase();
      const proj = (row[idxProj] || '').toString().toUpperCase();
      const qty = row[idxQty] || 0;

      // Skip empty bins or invalid bin codes
      const parsed = parseBinCode_(binCode);
      if (!parsed) continue;

      // Skip recently counted
      const key = [binCode, fbpn, man, proj].join('||');
      if (recentlyCountedSet.has(key)) continue;
      ...
      availableBins.push(binInfo);

      // Organize by row and level
      if (!rowMap[parsed.row]) {
        rowMap[parsed.row] = { levels: {} };
      }
      if (!rowMap[parsed.row].levels[parsed.level]) {
        rowMap[parsed.row].levels[parsed.level] = [];
      }
      rowMap[parsed.row].levels[parsed.level].push(binInfo);
    }

    if (availableBins.length === 0) {
      return {
        success: false,
        message: 'No available bins to count. All may have been counted recently.'
      };
    }

    // Get unique rows and shuffle them randomly
    const rows = Object.keys(rowMap);
    shuffleArray_(rows);

    // Select bins using S-pattern
    const selectedBins = [];
    let direction = 1; // 1 = ascending (low to high), -1 = descending (high to low)
    let rowIndex = 0;

    while (selectedBins.length < dailyTarget && rowIndex < rows.length * 10) {
      // Cycle through rows
      const currentRow = rows[rowIndex % rows.length];
      const rowData = rowMap[currentRow];
      ...
      // Get all available levels for this row and shuffle them (random level selection)
      const levels = Object.keys(rowData.levels).map(Number);
      shuffleArray_(levels);

      for (const level of levels) {
        const binsAtLevel = rowData.levels[level];
        if (!binsAtLevel || binsAtLevel.length === 0) continue;

        // Sort by sequence number based on current direction
        const sortedBins = [...binsAtLevel].sort((a, b) => {
          return direction === 1
            ? a.parsed.sequence - b.parsed.sequence
            : b.parsed.sequence - a.parsed.sequence;
        });

        // Take bins from this level
        for (const bin of sortedBins) {
          // Check if already selected
          const alreadySelected = selectedBins.some(
            s => s.binCode === bin.binCode &&
              s.fbpn === bin.fbpn &&
              s.manufacturer === bin.manufacturer &&
              s.project === bin.project
          );
          ...
          if (!alreadySelected) {
            ...
            const idx = binsAtLevel.indexOf(bin);
            if (idx > -1) binsAtLevel.splice(idx, 1);

            if (selectedBins.length >= dailyTarget) break;
          }
        }
      ...
      // Alternate direction for next row (S-pattern)
      direction *= -1;
      rowIndex++;

      // Safety: exit if we've cycled through all rows multiple times
      if (rowIndex > rows.length * 5 && selectedBins.length > 0) break;
    }

    if (selectedBins.length === 0) {
      return {
        success: false,
        message: 'Could not select any bins using S-pattern.'
      };
    }

    // Create the batch
    const batchLines = selectedBins.map(bin => ({
      binCode: bin.binCode,
      fbpn: bin.fbpn,
    ...
    if (!batchResult.success) {
      return {
        success: false,
        message: 'Failed to create batch: ' + (batchResult.message || 'Unknown error')
      };
    }
    ...
    return {
      ...
      bins: selectedBins.map(b => ({
        ...
        sequence: b.parsed.sequence
      })),
      message: `Created batch ${batchResult.batchId} with ${selectedBins.length} bins using S-pattern.`
    };

  } catch (err) {
    Logger.log('generateDailyCycleCountBatch error: ' + err.toString());
    return { success: false, message: 'Error: ' + err.message };
  }
}
...
/**
 * Gets the S-pattern sorted bins for the daily cycle count.
 * This is the function called by the modal for loading today's count.
 * 
 * @param {Object} filters - Optional filters
 * @returns {Array} Bins sorted in S-pattern order
 */
function getBinsForCycleCountSPattern(filters) {
  const result = generateDailyCycleCountBatch({
    dailyTarget: (filters && filters.dailyTarget) || 75,
    excludeRecent: true
  });

  if (!result.success) {
    // Fall back to regular method if S-pattern fails
    Logger.log('S-pattern failed, falling back: ' + result.message);
    return imsGetCycleCountBins(filters || {});
  }

  // Return bins in the order they were selected (S-pattern)
...
function imsCreateCycleCountBatch(payload) {
  ...
  const idxUOM = cycleHeaders.indexOf('UOM');
