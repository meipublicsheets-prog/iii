...
/**
 * Upsert a Customer PO into PO_Master with headers: Customer_PO, Project
 * Convention: headers row 1, spacer row 2, data starts row 3.
 * - If PO exists, updates Project.
 * - If PO does not exist, inserts new row at top (row 3).
 */
function upsertCustomerPO(customerPO, project) {
  const po = (customerPO || '').toString().trim();
  const proj = (project || '').toString().trim();
  if (!po) throw new Error('Customer PO is required.');
  if (!proj) throw new Error('Project is required.');

  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheetName = (typeof TABS !== 'undefined' && TABS.PO_MASTER) ? TABS.PO_MASTER : 'PO_Master';
  ...
  // Ensure headers exist (row 1) and spacer row 2
  const lastRow = sh.getLastRow();
  if (lastRow < 1) {
    sh.getRange(1, 1, 1, 2).setValues([['Customer_PO', 'Project']]);
    sh.insertRowAfter(1); // row 2 spacer
  } else {
    const hdr = sh.getRange(1, 1, 1, 2).getValues()[0];
    const needsHeader = !hdr[0] || hdr[0].toString().trim() !== 'Customer_PO' || !hdr[1] || hdr[1].toString().trim() !== 'Project';
    if (needsHeader) {
      sh.getRange(1, 1, 1, 2).setValues([['Customer_PO', 'Project']]);
    }
  ...
  if (dataLast < 3) {
    // no data yet, just write first row at row 3
    sh.getRange(3, 1, 1, 2).setValues([[po, proj]]);
    return { ok: true, action: 'inserted', row: 3 };
  }

  const range = sh.getRange(3, 1, dataLast - 2, 2);
  const values = range.getValues();

  for (let r = 0; r < values.length; r++) {
    const existingPO = values[r][0] ? values[r][0].toString().trim() : '';
    if (existingPO && existingPO === po) {
      // Update project only
      sh.getRange(3 + r, 2).setValue(proj);
      return { ok: true, action: 'updated', row: 3 + r };
    }
  }

  // Insert at top of data section (row 3)
 ...
 */
function getItemMasterDetails(sku, fbpn) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sheetName = TABS.ITEM_MASTER || 'Item_Master';
    ...
    Logger.log('Error in getItemMasterDetails: ' + error.toString());
    return { uom: 'EA', assetType: '' };
  }
}
...
function processInboundSubmission(payload) {
  ...
  // Look up Project from PO_Master based on Customer PO Number
  let projectFromPO = '';
  if (basic.customerPO) {
    projectFromPO = lookupProjectFromPO(basic.customerPO);
  }
  // Use project from PO lookup, fallback to form-provided project
  const resolvedProject = projectFromPO || basic.project || '';
  ...
  skidsData.forEach((skidEntry, index) => {
    ...
    skidItems.forEach((item, itemIndex) => {
      ...
      const itemDetails = getItemMasterDetails(sku, fbpn);
      const uom = itemDetails.uom || 'EA';
      const assetType = itemDetails.assetType || '';

      // Use resolved project from PO lookup (or fallback to form-provided)
      ...
      if (sIdx('Asset_Type') > -1) sRow[sIdx('Asset_Type')] = assetType;
      if (sIdx('UOM') > -1) sRow[sIdx('UOM')] = uom;
      ...
      if (stIdx('UOM') > -1) stRow[stIdx('UOM')] = uom;
      if (stIdx('Last_Updated') > -1) stRow[stIdx('Last_Updated')] = now;
  ...
  for (const fbpn in masterAggregation) {
    ...
    if (mIdx('UOM') > -1) mRow[mIdx('UOM')] = item.uom || 'EA';
...
function generateSkidLabelsHtml_(labelData) {
  ...
  let html = `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    ...
    * { box-sizing: border-box; }
    ...
    .top-left { flex: 1.4; }
    .manufacturer { font-size: 26pt; line-height: 1.05; text-transform: uppercase; }
    ...
    .top-barcode { width: 100%; height: 0.6in; margin-bottom: 0.02in; }
    .sku-text { font-size: 10pt; text-align: center; margin-top: 0.02in; color: #333; }
    ...
    .middle-block { margin-top: 0.06in; }
    ...
    .bottom-barcode { width: 100%; height: 0.55in; padding: 2px 25px; box-sizing: border-box; overflow: hidden; display: flex; justify-content: center; }
    .skid-id-text { font-size: 14pt; text-align: center; margin-top: 0.03in; font-weight: bold; }
  </style></head><body>`;

  labelData.forEach(d => {
    const skidValue = String(d.skidId || '');
    const skuValue = String(d.sku || '');

    // Top barcode = SKU (smaller, reference barcode)
    const skuBarcodeUri = bwipPngDataUri_('code128', skuValue, { scale: 3, height: 12 });

    // Bottom barcode = Skid ID (large, main scanning barcode)
    const bottomCodeType = base ? 'qrcode' : 'code128';
    const deepLink = base ? `${base}?skid=${encodeURIComponent(skidValue)}` : skidValue;
    const skidBarcodeUri = bwipPngDataUri_(bottomCodeType, deepLink, { scale: 4, height: 16 });
    ...
    <div class="label">
      <div class="top-row">
        ...
        <div class="top-right">
          ...
          <div class="sku-text">${escapeHtml_(skuValue)}</div>
        </div>
      </div>
      ...
      <div class="bottom-block">
        ...
        <div class="skid-id-text">${escapeHtml_(skidValue)}</div>
      </div>
    </div>`;
  });
...
// NOTE: escapeHtml_() moved to IMS_Config.js for shared use
