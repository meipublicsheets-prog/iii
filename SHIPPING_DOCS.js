...
function writeOutboundLogEntry(ss, data) {
  ...
  if (!mfr) {
    ...
    if (stSheet) {
      ...
      for (let i = 1; i < vals.length; i++) {
        if (String(vals[i][4] || '').trim() === String(data.fbpn || '').trim()) { // FBPN is now at index 4
          mfr = vals[i][2] || ''; // Manufacturer is now at index 2
          break;
        }
      }
    }
  ...
  let uom = data.uom || 'EA';
  if (!data.uom && (data.sku || data.fbpn)) {
    try {
      const itemSheet = ss.getSheetByName((typeof TABS !== 'undefined' && TABS.ITEM_MASTER) ? TABS.ITEM_MASTER : 'Item_Master');
      if (itemSheet) {
        ...
        if (skuIdx > -1 && data.sku) {
          const skuUpper = String(data.sku).toUpperCase().trim();
          for (let i = 1; i < itemData.length; i++) {
            const rowSku = String(itemData[i][skuIdx] || '').toUpperCase().trim();
            if (rowSku === skuUpper) {
              uom = uomIdx > -1 ? String(itemData[i][uomIdx] || 'EA') : 'EA';
              found = true;
              break;
            }
          }
      ...
      Logger.log('Error looking up Item_Master for UOM in writeOutboundLogEntry: ' + e.toString());
    }
  }
...
function updateCustomerOrderShipment(ss, orderId, info) {
  ...
  const cTotalSkids = _col_(h, ['total_skids']);
  ...
  for (let i = 1; i < data.length; i++) {
    if (_orderKey_(data[i][cOrder]) === _orderKey_(orderId)) {
      ...
      if (cTotalSkids >= 0 && info.totalSkids) sheet.getRange(i + 1, cTotalSkids + 1).setValue(info.totalSkids);
      break;
    }
  }
